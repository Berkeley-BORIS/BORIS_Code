...
## since I think we should update everything to work in the current coordinate system (Z positive in front of camera)
## Currently results are printed to terminal and saved to txt files, need to update this
## for however we want to do it in BORIS

def stereocalibration(check_img_folder,nimages=100,display=False,dims=(4,11),size=(640,480)):
	...
	for n in image_inds:
		...
		filename1 = check_img_folder + 'cam1_frame_'+str(n)+'.bmp'
		filename2 = check_img_folder + 'cam2_frame_'+str(n)+'.bmp'

		# if both files exist
		if os.path.exists(filename1) and os.path.exists(filename2):

			# read in the images and look for the circle grid points
			[img1,found1,points1] = get_circle_points(filename1,dims)
			[img2,found2,points2] = get_circle_points(filename2,dims)
			...
			if found1 and found2:
				...
				save_image_with_grid_drawn(n,img1,dims,points1,found1,check_img_folder)
				save_image_with_grid_drawn(n,img2,dims,points2,found2,check_img_folder)
...
def get_circle_points(filename,dims):
